...
def logits_to_video_file(logits, audio_filename, video_filename, labels=ppgs.PHONEME_LIST):
    ...
    composite = mpy.CompositeVideoClip([clip, overlay])
    composite = composite.set_audio(audio_clip)

    composite.write_videofile(video_filename)



if __name__ == '__main__':
    audio_filename = './tmp/arctic_a0001.wav'
    
    logits = ppgs.from_file(audio_filename, checkpoint='./runs/basemodel/00300000.pt', gpu=0).T

    logits_to_video_file(logits.cpu(), audio_filename, video_filename='./tmp/arctic_a0001.mp4')
